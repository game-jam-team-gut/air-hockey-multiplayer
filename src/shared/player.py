...
class Player:
    def __init__(self, s_x=sc.STRIKER_START_X, s_y=sc.PLAYER_STRIKER_START_Y, p_x=sc.WINDOW_WIDTH / 2,
                 p_y=sc.WINDOW_HEIGHT / 2, p_velocity=Vec2d(0.0, 0.0), pl_score=0, en_score=0,
                 is_primary_sync=False) -> None:
        self.s_x = s_x
        self.s_y = s_y
        self.p_x = p_x
        self.p_y = p_y
    ...
    def primary_map_to_enemy_side(self):
        return Player(sc.WINDOW_WIDTH - self.s_x, sc.WINDOW_HEIGHT - self.s_y,
                      sc.WINDOW_WIDTH - self.p_x, sc.WINDOW_HEIGHT - self.p_y, self.p_velocity, self.pl_score,
                      self.en_score, False)

    def secondary_map_to_enemy_side(self):
        return Player(sc.WINDOW_WIDTH - self.s_x, sc.WINDOW_HEIGHT - self.s_y, None, None, None, None, None, True)
