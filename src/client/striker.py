...
class Striker(PhysicsGameObject):
    ...
    def change_by_mouse_position(self, m_pos, movement_area):
        ...
        if movement_area.contains(m_rect):
            ...
            if movement_area.contains(m_y_rect):
                self.set_position((m_y_rect.centerx, m_y_rect.centery))

    def check_player_input(self, player_input, movement_area):
        old_pos = self.get_position()
        if player_input.dragging:
            m_pos = pygame.mouse.get_pos()
            if self.is_dragged or self.rect.collidepoint(m_pos):
                self.change_by_mouse_position(m_pos, movement_area)
